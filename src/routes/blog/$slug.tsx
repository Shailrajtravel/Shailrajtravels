...
import { Calendar, Clock, ChevronRight, User, Share2, Facebook, Twitter, Linkedin, ArrowRight } from 'lucide-react';
import { AuthorProfile } from '../../frontend/components/AuthorProfile';
import { generateArticleSchema, generateBreadcrumbSchema, generateFAQSchema } from '../../backend/lib/blog-schema';
        ...
        <div className="bg-white border-b border-slate-200 py-12 md:py-20 px-4 md:px-8">
          <div className="max-w-[800px] mx-auto text-center animate-reveal">
            ...
            <div className="flex items-center justify-center gap-2 text-sm text-slate-500 mb-8 flex-wrap">
              ...
              <ChevronRight className="w-3.5 h-3.5" />
              <Link to={`/blog/category/$categorySlug`} params={{ categorySlug: post.category.toLowerCase().replace(/\s+/g, '-') }} className="text-brand-blue font-medium hover:underline">
                {post.category}
              </Link>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-center gap-y-4 gap-x-8 text-slate-500 font-medium text-sm md:text-base">
              {author && (
                <div className="flex items-center gap-3">
                  {author.avatarUrl ? (
                    <img src={author.avatarUrl} alt={author.name} className="w-10 h-10 rounded-full border border-slate-200" />
                  ) : (
          ...
          <article className="lg:col-span-8 bg-white rounded-3xl p-6 md:p-12 shadow-sm border border-slate-100">
            ...
            {post.faqs && post.faqs.length > 0 && (
              <div className="mt-16 pt-12 border-t border-slate-100">
                <h2 className="text-3xl font-display font-bold text-brand-blue-deep mb-8">Frequently Asked Questions</h2>
            ...
            <div className="mt-16 pt-8 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-6">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {post.tags.map(tag => (
                  <Link key={tag} to={`/blog/tag/$tagSlug`} params={{ tagSlug: tag.toLowerCase().replace(/\s+/g, '-') }} className="px-3 py-1 bg-slate-100 text-slate-600 text-sm font-medium rounded-lg hover:bg-brand-blue hover:text-white transition-colors">
                    #{tag}
                  </Link>
                ))}
              </div>
              ...
              <div className="flex items-center gap-3">
                <span className="text-sm font-bold text-slate-400 uppercase tracking-wider mr-2">Share:</span>
                <a href={`https://www.facebook.com/sharer/sharer.php?u=${shareUrl}`} target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-blue-600 hover:text-white transition-colors">
                  <Facebook className="w-4 h-4" />
                </a>
                <a href={`https://twitter.com/intent/tweet?url=${shareUrl}&text=${shareTitle}`} target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-sky-500 hover:text-white transition-colors">
                  <Twitter className="w-4 h-4" />
                </a>
                <a href={`https://www.linkedin.com/shareArticle?mini=true&url=${shareUrl}&title=${shareTitle}`} target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-blue-700 hover:text-white transition-colors">
          ...
          <aside className="lg:col-span-4 space-y-8 sticky top-32">
            ...
            <div className="bg-brand-blue-deep rounded-3xl p-8 text-white relative overflow-hidden shadow-xl shadow-brand-blue-deep/20">
              ...
              <p className="text-brand-blue-light/80 text-sm mb-6 relative z-10">
                Let Shailraj Travels guide you on a hassle-free, comfortable, and spiritually fulfilling pilgrimage.
              </p>
              
              {post.relatedTourSlugs && post.relatedTourSlugs.length > 0 ? (
                <div className="space-y-3 relative z-10">
                  {post.relatedTourSlugs.slice(0, 2).map((tourSlug, idx) => {
                    const tour = require('../../frontend/features/tours/data').tours.find((t: any) => t.slug === tourSlug);
                    return (
                      <Link key={idx} to={`/tours/$tourSlug`} params={{ tourSlug }} className="flex flex-col items-start justify-between w-full p-4 bg-white/10 hover:bg-white/20 rounded-xl transition-colors backdrop-blur-sm group">
                        ...
                        <span className="font-medium text-[15px] truncate pr-4 text-white">{tour ? tour.title : 'View Tour Details'}</span>
                      </Link>
                    );
                  })}
                </div>
              ) : (
                <a href="/#tours" className="inline-flex items-center justify-center gap-2 w-full px-6 py-3 bg-brand-green hover:bg-brand-green-dark text-brand-blue-deep font-bold rounded-xl transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5 relative z-10">
              ...
              <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm">
                ...
                <div className="space-y-6">
                  {relatedArticles.map(article => (
                    <Link key={article.slug} to={`/blog/$slug`} params={{ slug: article.slug }} className="group block">

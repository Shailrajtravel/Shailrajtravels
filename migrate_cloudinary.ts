...
async function migrate() {
  ...
  for (const photo of gallery) {
    if (photo.imageUrl && photo.imageUrl.startsWith("data:image")) {
      ...
      try {
        const res = await cloudinary.uploader.upload(photo.imageUrl, {
          folder: "gallery",
          resource_type: "auto",
        });
        await db
          .collection("gallery")
          .updateOne({ _id: photo._id }, { $set: { imageUrl: res.secure_url } });
  ...
  for (const pkg of packages) {
    ...
    if (newData.images && Array.isArray(newData.images)) {
      newData.images = await Promise.all(
        newData.images.map(async (img: string, idx: number) => {

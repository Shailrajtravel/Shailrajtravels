      ...
      <div className="flex flex-col space-y-4">
        {blogs.map((blog) => (
          <a key={blog.slug} href={`/blog/${blog.slug}`} className="group flex items-start">
            ...
            <div>
              <h4 className="font-semibold text-gray-900 group-hover:text-brand-orange transition-colors leading-tight">
                {blog.title}
              </h4>
              <p className="text-sm text-brand-orange mt-1 group-hover:underline">
                Read Article &rarr;
              </p>
            </div>
          </a>

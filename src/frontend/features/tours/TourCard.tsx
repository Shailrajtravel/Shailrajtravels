    ...
    <div className="bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 flex flex-col max-w-[400px] w-full mx-auto h-full">
      ...
      <div className="relative h-[220px] w-full overflow-hidden">
        ...
        <div className="absolute bottom-4 left-5 right-5 z-10">
          <p className="text-[#F59E0B] font-medium text-[13px] mb-1 drop-shadow-sm">{tour.subtitle}</p>
          <h3 className="text-white font-bold text-2xl leading-tight drop-shadow-md">{tour.title}</h3>
        </div>
      </div>
      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        <div className="flex flex-wrap gap-2 mb-5">
          {(tour.tags || []).map((tag, index) => (
            <span key={index} className="bg-slate-100 text-slate-700 text-[13px] font-semibold px-3 py-1.5 rounded-full">
              {tag}
            </span>
          ))}
        </div>
        ...
        <div className="mb-5 mt-auto">
          <div className="flex justify-between items-center text-[13px] font-semibold text-slate-500 mb-2">
            <span>{tour.seatsAvailable} {t?.cardSeatsAvail || "seats available"}</span>
            <span>{tour.seatsTotal} {t?.cardSeatsTotal || "total"}</span>
          </div>

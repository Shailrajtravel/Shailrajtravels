...
export const Route = createFileRoute("/pilgrimage-guides")({
  head: () => ({
    meta: generateSEO({
      ...
      description:
        "Explore comprehensive guides, travel tips, and route maps for Ashtavinayak, Char Dham, Jyotirlinga, and Shirdi pilgrimages.",
      ...
      <div className="max-w-6xl mx-auto space-y-12">
        <div className="text-center max-w-3xl mx-auto">
          ...
          <p className="text-xl text-slate-600">
            Comprehensive resources, route maps, and travel advice for your sacred journeys across
            India.
          </p>
        </div>
          ...
          <div className="flex flex-wrap gap-3">
            {Object.keys(clusters).map((key) => (
              <a
                ...
                className="px-4 py-2 bg-brand-blue/5 text-brand-blue-deep rounded-full text-sm font-semibold hover:bg-brand-blue/10 transition-colors"
              >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {posts.map((post) => (
                  <Link
                    key={post.slug}
                    to={`/blog/${post.slug}`}
                    className="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100 hover:shadow-lg transition-all group flex flex-col h-full"
                  >
                    ...
                    <div className="p-6 flex-1 flex flex-col">
                      ...
                      <h3 className="text-lg font-bold text-slate-900 mb-3 line-clamp-2 group-hover:text-brand-blue-deep transition-colors">
                        {post.title}
                      </h3>

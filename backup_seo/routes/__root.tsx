    ...
    ({
      meta: [
        ...
        ...generateSEO({
          ...
          description:
            "Trusted Pilgrimage Tour Operator in Pune. Book Ashtavinayak, Jyotirlinga, Pandharpur, and Char Dham yatra packages with AC travel and guided darshan.",
...
function RootShell({ children }: { children: ReactNode }) {
  // Try to use location, but RootShell might be rendered before router is fully initialized in some SSR setups.
  // We'll safely fallback to en.
  let isMarathi = false;
  try {
    const location = useLocation();
    isMarathi = location.pathname.startsWith("/mr");
  } catch (e) {}
...
function LanguageToggle({ isMarathi, currentPath }: { isMarathi: boolean; currentPath: string }) {
  const router = useRouter();

  const toggleLanguage = () => {
    if (isMarathi) {
      // Switch to English
      const newPath = currentPath.replace(/^\/mr/, "") || "/";
      router.navigate({ to: newPath });
    } else {
      // Switch to Marathi
      const newPath = `/mr${currentPath === "/" ? "" : currentPath}`;
      router.navigate({ to: newPath });
    }
  };
...
function RootComponent() {
  ...
  const isMarathi = location.pathname.startsWith("/mr");

  // Ensure html lang is set even if RootShell useLocation fails
  useEffect(() => {
  ...
  return (
    <QueryClientProvider client={queryClient}>
      ...
      {!isAdmin && (
        <>
          <LanguageToggle isMarathi={isMarathi} currentPath={location.pathname} />
          <FloatingWhatsApp />
        </>
      )}
    </QueryClientProvider>

          ...
          <Link key={tour.slug} to="/tours/$tourSlug" params={{ tourSlug: tour.slug! }} className="block group">
            <div className="border border-gray-200 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow">
              <div className="bg-gray-100 h-32 flex items-center justify-center relative overflow-hidden">
                {tour.image ? (
                  <LazyImage src={tour.image} alt={tour.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                ) : (
              ...
              <div className="p-4 bg-white">
                <h4 className="font-semibold text-gray-900 group-hover:text-brand-orange transition-colors">{tour.title}</h4>
              </div>
            </div>
          </Link>
        ))}

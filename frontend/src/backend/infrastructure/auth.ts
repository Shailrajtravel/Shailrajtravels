...
export const verifyAdminFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (data.email && data.password) {
      const expectedEmail = (import.meta as any).env?.VITE_ADMIN_EMAIL || process.env.ADMIN_EMAIL || "khudeshivam@gmail.com";
      const hash = (import.meta as any).env?.VITE_ADMIN_PASSWORD_HASH || process.env.ADMIN_PASSWORD_HASH;

...
const frontendTargets = ["assets", "components", "config", "data", "features", "hooks", "invioce", "templates", "types"];
...
for (const file of allFiles) {
  ...
  for (const f of frontendTargets) {
    ...
    content = content.replace(new RegExp(`from\\s+['"]\\.\\./${f}/`, 'g'), `from '../frontend/${f}/`);
    content = content.replace(new RegExp(`from\\s+['"]\\.\\./\\.\\./${f}/`, 'g'), `from '../../frontend/${f}/`);
    content = content.replace(new RegExp(`import\\(['"]\\.\\./${f}/`, 'g'), `import('../frontend/${f}/`);
  }

  for (const b of backendTargets) {
    if (b === 'server.ts') continue;
    content = content.replace(new RegExp(`@/${b}/`, 'g'), `@/backend/${b}/`);
    content = content.replace(new RegExp(`from\\s+['"]\\.\\./${b}/`, 'g'), `from '../backend/${b}/`);
    content = content.replace(new RegExp(`from\\s+['"]\\.\\./\\.\\./${b}/`, 'g'), `from '../../backend/${b}/`);
    content = content.replace(new RegExp(`import\\(['"]\\.\\./${b}/`, 'g'), `import('../backend/${b}/`);
  }

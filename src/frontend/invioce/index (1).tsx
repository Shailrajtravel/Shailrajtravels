    ...
    <div className="min-h-screen bg-[#eef0f3] py-6 print:p-0 print:bg-white">
      ...
      <div className="mx-auto flex w-[210mm] justify-end gap-3 mb-4 no-print">
        {isEditing ? (
          <button
            onClick={handleSave}
            className="flex items-center gap-2 rounded bg-green-600 px-4 py-2 font-bold text-white shadow hover:bg-green-700 transition"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 rounded bg-[#0B3D91] px-4 py-2 font-bold text-white shadow hover:bg-blue-800 transition"
          >
        ...
        <button
          ...
          className="flex items-center gap-2 rounded bg-slate-800 px-4 py-2 font-bold text-white shadow hover:bg-slate-900 transition"
        >
...
function Invoice({ data, setData, isEditing }: any) {
  ...
  return (
    ...
    >
      ...
      {/* CONTACT ROW */}
      <div
        className="mt-3 flex flex-wrap items-center justify-center gap-x-6 gap-y-1 text-[13px] font-medium"
        style={{ color: "#222" }}
      >
        <span className="flex items-center gap-1.5">
          <MapPin className="h-4 w-4" style={{ color: DARK }} /> Pune, Maharashtra, India
        </span>
        <span style={{ color: "#bbb" }}>|</span>
        <span className="flex items-center gap-1.5">
          <Phone className="h-4 w-4" style={{ color: DARK }} /> +91 97634 33556
        </span>
      </div>
      <div className="mt-1 flex flex-wrap items-center justify-center gap-x-6 gap-y-1 text-[13px] font-medium">
        <span className="flex items-center gap-1.5">
          <Mail className="h-4 w-4" style={{ color: DARK }} /> shailrajtravels9999@gmail.com
        </span>
        <span style={{ color: "#bbb" }}>|</span>
        <span className="flex items-center gap-1.5">
          <Globe className="h-4 w-4" style={{ color: DARK }} /> www.shailrajtravels.com
        </span>
      </div>
        ...
        <div className="grid grid-cols-2">
          ...
          <div className="px-6 py-4">
            ...
            <InfoLine
              ...
              value={data.dueDate}
              isEditing={isEditing}
              onChange={(v: string) => updateData("dueDate", v)}
            />
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-4">
        <Card title="BILL TO">
          <DetailRow
            ...
            isEditing={isEditing}
            onChange={(v: string) => updateData("billToName", v)}
          />
          <DetailRow
            label="Mobile"
            value={data.billToMobile}
            isEditing={isEditing}
            onChange={(v: string) => updateData("billToMobile", v)}
          />
          <DetailRow
            label="Email"
            value={data.billToEmail}
            isEditing={isEditing}
            onChange={(v: string) => updateData("billToEmail", v)}
          />
          <div className="h-16" />
        </Card>
        <Card title="TRIP DETAILS">
          ...
          <DetailRow
            ...
            value={data.duration}
            isEditing={isEditing}
            onChange={(v: string) => updateData("duration", v)}
          />
          <DetailRow
            label="Travel Date"
          ...
          <DetailRow
            ...
            isEditing={isEditing}
            onChange={(v: string) => updateData("vehicleType", v)}
          />
          <DetailRow
            label="Pickup Point"
        ...
        <table className="w-full border-collapse text-[13px]">
          <thead>
            <tr style={{ background: DARK, color: "white" }}>
              ...
              <th className="w-[120px] px-5 py-3 text-center text-[13px] font-bold uppercase">
                Qty
              </th>
              <th className="w-[140px] px-5 py-3 text-center text-[13px] font-bold uppercase">
                Rate (₹)
              </th>
              <th className="w-[160px] px-5 py-3 text-right text-[13px] font-bold uppercase">
                Amount (₹)
              </th>
            </tr>
          </thead>
          <tbody>
            <tr className="border-t" style={{ borderColor: BORDER }}>
              <td className="px-5 py-3">
                {isEditing ? (
                  <input
                    ...
                    value={data.itemDesc}
                    onChange={(e) => updateData("itemDesc", e.target.value)}
                  />
                ) : (
                  data.itemDesc
                )}
              ...
              <td className="px-5 py-3 text-center">
                {isEditing ? (
                  <input
                    className="w-full text-center border border-slate-300 rounded px-1 py-0.5 outline-none focus:ring-1 focus:ring-brand-blue"
                    value={data.itemQty}
                    onChange={(e) => updateData("itemQty", e.target.value)}
                  />
                ) : (
                  data.itemQty
                )}
              ...
              <td className="px-5 py-3 text-center">
                {isEditing ? (
                  <input
                    className="w-full text-center border border-slate-300 rounded px-1 py-0.5 outline-none focus:ring-1 focus:ring-brand-blue"
                    value={data.itemRate}
                    onChange={(e) => updateData("itemRate", e.target.value)}
                  />
                ) : (
                  data.itemRate
                )}
              ...
              <td className="px-5 py-3 text-right">
                {isEditing ? (
                  <input
                    className="w-full text-right border border-slate-300 rounded px-1 py-0.5 outline-none focus:ring-1 focus:ring-brand-blue"
                    value={data.itemAmount}
                    onChange={(e) => updateData("itemAmount", e.target.value)}
                  />
                ) : (
                  data.itemAmount
                )}
            ...
            <tr className="border-t" style={{ borderColor: BORDER }}>
              ...
              <td
                className="px-5 py-4 text-right text-[15px] font-bold uppercase"
                style={{ color: DARK }}
              >
                Total Amount
              </td>
              <td
                className="px-5 py-4 text-right text-[26px] font-extrabold"
                style={{ color: DARK }}
              >
                {isEditing ? (
                  <div className="flex items-center justify-end gap-1">
                    <span>₹</span>
                    <input
                      className="w-32 text-right border border-slate-300 rounded px-1 py-0.5 text-lg font-bold outline-none focus:ring-1 focus:ring-brand-blue"
      ...
      <div className="mt-4 grid grid-cols-2 gap-4">
        <Card title="PAYMENT DETAILS">
          <DetailRow
            ...
            isEditing={isEditing}
            onChange={(v: string) => updateData("paymentMode", v)}
          />
          <DetailRow
            label="Paid Amount"
            value={data.paidAmount}
            isEditing={isEditing}
            onChange={(v: string) => updateData("paidAmount", v)}
          />
          <DetailRow
            label="Balance Amount"
            value={data.balanceAmount}
            isEditing={isEditing}
            onChange={(v: string) => updateData("balanceAmount", v)}
          />
          <div className="mt-2 flex items-center text-[13px]">
            ...
            {isEditing ? (
              <input
                className="ml-2 flex-1 border border-slate-300 rounded px-1 py-0.5 outline-none focus:ring-1 focus:ring-brand-blue"
              ...
              <span
                ...
                style={{
                  background: data.paymentStatus?.toLowerCase() === "pending" ? "#F59E0B" : GREEN,
                }}
              >
                {data.paymentStatus}
              </span>
      ...
      >
        <span className="flex items-center gap-2">
          <Phone className="h-4 w-4" /> +91 97634 33556
        </span>

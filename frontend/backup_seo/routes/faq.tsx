...
export const Route = createFileRoute("/faq")({
  head: () => ({
    meta: generateSEO({
      ...
      description:
        "Frequently asked questions about our pilgrimage tours. Learn about booking, cancellation, physical fitness, and what to pack.",
...
function FAQPage() {
  ...
  const allFaqs = tours.flatMap((t) =>
    t.faq.map((f) => ({ ...f, tourTitle: t.title, tourSlug: t.slug })),
  );
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-slate-600 mb-8">
            Everything you need to know about our pilgrimage packages.
          </p>
          ...
          {filteredFaqs.length > 0 ? (
            filteredFaqs.map((faq, idx) => (
              <div key={idx} className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
                <Link
                  to={`/tours/${faq.tourSlug}`}
                  className="text-xs font-bold text-brand-orange uppercase tracking-wider mb-2 hover:underline block"
                >
                  {faq.tourTitle} &rarr;
                </Link>
            ...
            <div className="text-center py-12 text-slate-500">
              No questions found matching your search. Please try a different term or contact us
              directly.
            </div>
          )}
        </div>

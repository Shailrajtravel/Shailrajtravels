...
${review.text || review.textEn || review.textMr}`;
    ...
    try {
      ...
      if (parsed.blogContent && parsed.blogContent.length > 20)
        updateData.blogContent = parsed.blogContent;
      if (parsed.blogTitleMr) updateData.blogTitleMr = parsed.blogTitleMr;
      if (parsed.blogContentMr && parsed.blogContentMr.length > 20)
        updateData.blogContentMr = parsed.blogContentMr;

      if (Object.keys(updateData).length > 0) {
        await db.collection("reviews").updateOne({ _id: review._id }, { $set: updateData });

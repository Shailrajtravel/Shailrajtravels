...
import { isUpcomingDate } from '@/backend/shared/bookings';
...
export const getTourBySlugFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    try {
      ...
      let tour = await tourRepository.findBySlug(data.slug, { lang: query.lang });

      // Fallback: if not found for the requested language, fall back to English/un-versioned
      if (!tour && data.lang) {
        tour = await tourRepository.findBySlug(data.slug, { lang: { $in: ["en", null] } });
      }

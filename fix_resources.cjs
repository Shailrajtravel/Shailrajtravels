...
f = f.replace(
  /import \{ tours \} from '\.\.\/\.\.\/data\/tours\.en';/g,
...
f = f.replace(
  /loader: \(\{ params \}\) => \{[\s\S]*?return resource;\n  \},/g,
  `loader: async ({ params }) => {
...
f = f.replace(
  /const resource = Route\.useLoaderData\(\);\n\s*const linkedTours = tours\.filter\(t => resource\.relatedTours\.includes\(t\.slug\)\);/g,
  "const { resource, linkedTours } = Route.useLoaderData() as any;",
);

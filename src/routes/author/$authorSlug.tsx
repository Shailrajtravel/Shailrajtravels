      ...
      <div className="max-w-5xl mx-auto space-y-12">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-8 md:p-12 flex flex-col md:flex-row gap-10 items-center md:items-start">
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <div className="text-brand-orange font-bold text-lg uppercase tracking-wider mb-6">
              {author.role}
            </div>

            <p className="text-slate-700 text-lg leading-relaxed mb-8">{author.bio}</p>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              {author.expertiseAreas && (
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                  <div className="font-bold text-slate-900 mb-2 flex items-center gap-2">
                    <Award className="w-4 h-4 text-brand-orange" /> Core Expertise
                  </div>
                  <ul className="text-slate-600 list-disc pl-5 space-y-1">
                    {author.expertiseAreas.map((exp, idx) => (
                      <li key={idx}>{exp}</li>
                    ))}
                  </ul>
                </div>
              ...
              {author.regionsCovered && (
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                  <div className="font-bold text-slate-900 mb-2 flex items-center gap-2">
                    <MapPin className="w-4 h-4 text-brand-orange" /> Regions Covered
                  </div>
                  <ul className="text-slate-600 list-disc pl-5 space-y-1">
                    {author.regionsCovered.map((reg, idx) => (
                      <li key={idx}>{reg}</li>
                    ))}
                  </ul>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {authorArticles.map((post) => (
                <Link
                  key={post.slug}
                  to={`/blog/${post.slug}`}
                  className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 hover:shadow-lg transition-all group flex flex-col h-full"
                >
                  <div className="text-xs font-bold text-brand-orange uppercase tracking-wider mb-2">
                    {post.category}
                  </div>
                  <h3 className="text-xl font-bold text-slate-900 mb-3 line-clamp-2 group-hover:text-brand-blue-deep transition-colors">
                    {post.title}
                  </h3>
